...
- Test: run the suites SEPARATELY (SDK 8.0.424 errors MSB1008 if both are given in one `dotnet test`):
  ...
  - `& "$env:LOCALAPPDATA\Microsoft\dotnet\dotnet.exe" test tests/PageForge.UiSmoke.Tests` (4 tests) — launches the REAL window and drives it through UI Automation, so it needs an interactive desktop session and `src/PageForge.App.Wpf` built in the SAME configuration. Runs in the `ui-smoke` CI lane; until that lane existed the suite was in no lane and not even in `PageForge.sln`, so its assertions protected nothing. **CI runs only 3 of the 4**: `Wpf_app_opens_organizes_and_saves_a_reordered_document` is filtered out there because it drives a native common file dialog through Win32 `SendMessage` (file dialogs are not exposed to UI Automation), and on the hosted image that silently does nothing — no error, no file. **Reorder-and-save is therefore NOT covered by CI: run this suite locally before trusting that path.**
