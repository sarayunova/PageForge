...
- Test: run the two suites SEPARATELY (SDK 8.0.424 errors MSB1008 if both are given in one `dotnet test`):
  - `& "$env:LOCALAPPDATA\Microsoft\dotnet\dotnet.exe" test tests/PageForge.Core.Tests` (141 tests)
  - `& "$env:LOCALAPPDATA\Microsoft\dotnet\dotnet.exe" test tests/PageForge.Fidelity.Tests` (42 tests)
