...
- Build: `powershell -ExecutionPolicy Bypass -File tools/build-managed.ps1` (add `-Configuration Release -WarnAsError` to match CI's gate). Do NOT build `PageForge.sln` directly: the solution contains `src/PageForge.App` (WinUI), this dev machine lacks the VS UWP/MSIX MSBuild tasks (`Microsoft.Build.AppxPackage.dll`, `Microsoft.Build.Packaging.Pri.Tasks.dll`), and the solution build therefore fails MSB4062 every time — the previously documented `build PageForge.sln` command could not run on the machine it documented. The script owns the managed project list and throws if `PageForge.sln` gains a project it does not know about, so the list cannot silently drift again. The WinUI spike builds only in the `winui-build` CI lane, which is enforcing (no `continue-on-error`) so the spike cannot rot.
