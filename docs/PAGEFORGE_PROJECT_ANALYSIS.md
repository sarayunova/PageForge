...
- `release.yml`: publish + zip payload unsigned (`publish-release.ps1`) → upload as build artifact → **SignPath** (`SignPath/github-action-submit-signing-request`) signs the zip server-side → signtool verify *before* the signed zip replaces the release asset → draft GitHub Release. Decided 2026-09-12: when signing isn't configured, a `v*` tag still creates a draft release with an unsigned payload whose release notes say so and warn about SmartScreen/Defender/browser "unknown publisher" prompts — unsigned beta ships over a withheld one (TSD §12.1; the signed path re-activates automatically once the SignPath secret/variables exist). Azure Artifact Signing was the original plan but its identity validation is unavailable for an India-based maintainer, so the backend switched to SignPath.

## 7. Key open items / risks (from the Phase 7 handoff)

1. **Signing — ship-unsigned decision RESOLVED (2026-09-12); backend switched to SignPath (2026-09-24).** `release.yml` gate relaxed, release notes made truthful (+ SmartScreen warnings), TSD §12.1 amended, handoff/docs updated. Azure Artifact Signing was the original plan but its identity validation is not available for an India-based maintainer; `release.yml`/`publish-release.ps1` now target **SignPath** (SignPath Foundation's free OSS code signing) instead — pre-wired and gated on `SIGNPATH_API_TOKEN`, still needing the maintainer's application to be approved (~1-2 weeks) and the resulting org/project/policy identifiers before signed installers take over distribution.
